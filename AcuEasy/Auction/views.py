...
class Current_Auction(View):
    ...
    def get(self, request, *arg):
        ...
        data = zip(self.objAuction, self.bidList, self.amount,self.is_seller_copy)
        # data1 = zip(self.objAuction, self.bidList, self.amount)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if currentForm.is_valid():
            ...
            if amount > lastbid_amount:
                ...
                bidd = None
                try:
                    user = User.objects.filter(user_id = uid,user_username__exact=uname).last()
                    ...
                    objBid.save()
                    return redirect('/auction/current-auction/')
        messages.error(request, "Please enter amount greater than last bid amount")
        return redirect('/auction/current-auction/')
